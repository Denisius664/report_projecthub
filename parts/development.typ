...
+ Безопасность: JWT-токены. 
 - Обеспечение авторизации без SSO.
...
- База данных: PostgreSQL с использованием расширений JSONB (для хранения гибких метаданных) и LTREE (для иерархий предметных областей). Структура нормализована (3НФ), включает таблицы для пользователей, проектов, предметных областей, файлов, связей между проектами и участников. Логическая схема БД (@fig:logicmodel).
...
- Загрузка проекта в допустимых форматах (текст, PDF, Word, Excel, JSON).
